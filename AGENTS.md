...
- **Disk housekeeping**: the workspace-cleanup step prunes Bazel output bases (`~/.cache/bazel`) untouched for 7 days — they grow unbounded across runs and are the #1 ENOSPC cause on persistent hosts (see failed run 33241700491). A variant-aware "Check free disk space" step then hard-fails self-hosted jobs below 50 GiB free, or below 150 GiB for the Bazel DDK variant `OP13-6.6.118` (its output base dominates; a 100 GB host cannot fit it at all). Hosted runners only warn; their ~35-45 GiB is historically sufficient.
