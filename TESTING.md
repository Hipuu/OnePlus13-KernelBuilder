...
- Every workflow and composite `action.yml` parses as YAML, and all seven device configs / seven manifests parse as JSON / XML.
- Every DDK unified-diff hunk contains exactly the old/new line counts declared by its header, including blank added lines.
- `actionlint` reports no problems (skipped with a notice if not installed). One rule is suppressed: actionlint through 1.7.7 still enforces a 10-input cap on `workflow_dispatch`, but GitHub's documented limit is 25 top-level inputs, and this workflow's 20 dispatch inputs fine.
...
Battery-first build (`battery_save` / `audit_off` opt-ins, see README):

```bash
gh workflow run "Build OnePlus 13 Kernel" -R Hipuu/OnePlus13-KernelBuilder \
  -f kernel_version="6.6.118 A16" -f battery_save=true -f audit_off=true
```

Verify on device after flashing such a build:

- `cat /proc/cmdline | grep -o 'audit=[01]'` shows `audit=0`, and `dmesg | grep 'audit: disabled'` appears once. (EXTEND order means bootloader args come last; for `audit=` specifically the first `audit=0` latches `audit_initialized=AUDIT_DISABLED` in `audit_enable` and `audit_init` then early-returns, so a later bootloader `audit=1` cannot re-enable audit — the check is belt-and-braces hygiene and also catches other bootloader token surprises.)
- `grep -o 'rcupdate.rcu_normal_after_boot=1' /proc/cmdline` matches.
- `zcat /proc/config.gz | grep WQ_POWER_EFFICIENT_DEFAULT` shows `=y` (on kernels exposing config.gz), or check the CI "Verify Bazel defconfig" log.
- After installing the service.d script: `/proc/sys/vm/page-cluster` reads `0` after reboot (was `3`), and `/sys/module/oplus_chg_v2/parameters/oplus_log_level` reads `1` (was `2`).
...
|-----|-----------|---------|
| `plan` | always | Maps the `kernel_version` choice to a JSON matrix of config paths (`all` expands to all seven) |
