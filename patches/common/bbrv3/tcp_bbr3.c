/*
 * Reproducible source provenance: extracted without semantic changes from
 * WildKernels/kernel_patches common/bbrv3/
 * 0001-net-tcp-backport-BBRv3-to-android15-6.6.patch at locked commit
 * 2ee34500cb4c3ee954ba36090e11f6ff08b3ec2f. The upstream MODULE_LICENSE
 * declaration is retained below.
 */
...
static void bbr3_skb_marked_lost(struct sock *sk, const struct sk_buff *skb);

static inline struct bbr3 *bbr3_get_priv(const struct sock *sk)
{
	/* Read the address value out of slot 0 of the 104-byte array space */
...
/* Like bbr2_tso_segs(), using mss_cache, ignoring driver's sk_gso_max_size. */
...
static enum tcp_bbr_phase bbr_get_phase(struct bbr3 *bbr)
{
	switch (bbr->mode) {
	case BBR_STARTUP:
